Feature: Delete a specific category
    ...
    Scenario Outline: Delete a category
        Given the category with id <id> exists in the system (US14)
    ...
    Scenario Outline: Delete a category that has an empty description
        Given the category with id <id> no description exists in the system (US14)
        And the following categories exist in the system (US14)
            | title | description  |
            | todo1 | description1 |
            | todo2 |              |

        When I delete the category without a description with id <id> (US14)
        Then the category without a description with id <id> should not exist in the system (US14)
    ...
    Scenario Outline: Delete a category that does not exist
        Given the category with invalid id "<id>" does not exist in the system (US14)
        ...
        Examples:
            ...
            | qwe | 404        | Could not find any instances with categories/qwe |